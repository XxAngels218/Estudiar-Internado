...
import { CheckCircle2, XCircle, RotateCcw, ChevronRight, Award, ArrowLeft } from 'lucide-react';
import { getAllTopics } from '../data/topics';

function ExamenCompleto() {
  ...
  // Obtener todas las preguntas de todos los temas
  const allTopics = getAllTopics();
  const allQuestions = allTopics.flatMap(topic => 
    topic.preguntas.map(pregunta => ({
      ...pregunta,
  ...
  // Función para seleccionar 50 preguntas aleatorias
  const selectRandomQuestions = (questions, count = 50) => {
    const shuffled = [...questions].sort(() => Math.random() - 0.5);
    return shuffled.slice(0, Math.min(count, questions.length));
  };

  const questions = examQuestions.length > 0 ? examQuestions : [];
  ...
  const handleStart = () => {
    const randomQuestions = selectRandomQuestions(allQuestions, 50);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 py-12 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center">
            <div className="mb-6">
              <div className="w-20 h-20 rounded-full mx-auto flex items-center justify-center text-4xl bg-gradient-to-br from-purple-100 to-indigo-100">
                🎓
              </div>
            </div>
            <h2 className="text-3xl font-bold text-gray-800 mb-4">
              Examen Completo de Internado
            </h2>
            <p className="text-lg text-gray-600 mb-8">
              Evalúa tus conocimientos con 50 preguntas de todos los temas
            </p>
            
            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-6 mb-8 text-left">
              <h3 className="font-semibold text-gray-800 mb-3">Instrucciones:</h3>
              <ul className="space-y-2 text-gray-600">
                <li>• Se seleccionarán 50 preguntas aleatorias del banco completo de {allQuestions.length} preguntas</li>
                <li>• Las preguntas incluyen los 5 temas: Pediatría, Ginecología, Medicina Interna, Cirugía y Temas Generales</li>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
              {allTopics.map(topic => (
                <div key={topic.id} className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="text-sm font-semibold text-gray-700">{topic.nombre}</div>
                  <div className="text-xs text-gray-500">{topic.preguntas.length} preguntas</div>
                </div>
              ))}
            </div>
            ...
            <button
              onClick={handleStart}
              className="px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold text-lg shadow-lg hover:shadow-xl transition-all transform hover:scale-105"
            >
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 sm:p-12">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">
                {passed 
                  ? 'Has demostrado un buen dominio general de los temas' 
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="flex-1 flex items-center justify-center px-6 py-3 border-2 border-purple-600 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition-colors"
              >
                Ver Respuestas
              </button>
              <button
                onClick={handleRestart}
                className="flex-1 flex items-center justify-center px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold hover:opacity-90 transition-opacity"
              >
            ...
            <button
              onClick={handleBackToHome}
              className="mt-4 w-full flex items-center justify-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 transition-colors"
            >
              <ArrowLeft className="w-5 h-5 mr-2" />
              Volver al Inicio
            </button>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 sm:p-8">
          ...
          <div className="mb-6">
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="h-full transition-all duration-300 bg-gradient-to-r from-purple-600 to-indigo-600"
                style={{
          ...
          <div className="mb-4">
            <span 
              ...
              style={{ backgroundColor: currentQuestion.topicColor }}
            >
              {currentQuestion.topicNombre}
            </span>
          </div>
            ...
            <div className="space-y-3">
              {currentQuestion.opciones.map((opcion, index) => {
                ...
                if (showCorrect) {
                  ...
                  optionClass = 'border-2 border-purple-600 bg-purple-50';
                }
                  ...
                  >
                    <div className="flex items-start">
                      <span 
                        ...
                        style={{
                          borderColor: isSelected && !showReview ? '#9333ea' : '#d1d5db',
                          backgroundColor: isSelected && !showReview ? '#9333ea' : 'white',
          ...
          <div className="flex justify-between items-center">
            ...
            {showReview ? (
              currentQuestionIndex < totalQuestions - 1 ? (
                <button
                  onClick={() => setCurrentQuestionIndex(currentQuestionIndex + 1)}
                  className="flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold hover:opacity-90 transition-opacity"
                >
                  Siguiente
              ...
              <button
                ...
                className={`flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold transition-all ${
